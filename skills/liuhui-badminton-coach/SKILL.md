...
- Do not treat an action performed only by a coach, partner, opponent, wall-feed setup, or shuttle machine as the user's requested technique. A broad source category such as `发球与接发` is navigation metadata, not proof that the video teaches both roles. A role-unspecified source may support only a separately requested generic component such as grip or push mechanics, never the serve or receive claim itself, and not when source wording suppresses or contradicts that role.
