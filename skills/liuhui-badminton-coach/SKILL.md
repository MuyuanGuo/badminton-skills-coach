...
5. Treat `claim_evidence_map` as the complete per-claim related-evidence allowlist and `selected_videos` as the global audit pool. `synthesis_videos` is the smaller allowlist for technical prose, `core_videos` is the bounded 3–5 item viewing shortlist, and `complete_related_videos` is the exhaustive packet-authorized list. Evidence permission never transfers between claims or layers.
...
In `核心视频与观看重点`, resolve `core_videos` against `selected_videos` and give each video's relevance, observation focus, timestamp/clip range when available, stable `evidence_id`, and canonical URL once. In `完整相关视频`, then list every remaining label from `complete_related_videos`, preserving its stable identity and canonical URL once. Never silently replace the complete list with the core shortlist, and never use a complete-list-only source to expand technical prose. If no related source is authorized, state the supported boundary or evidence gap.
