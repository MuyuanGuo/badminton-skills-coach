...
- Preserve active/passive, singles/doubles, forehand/backhand, level, and court-position distinctions. Treat `挑球` and `过渡球` as distinct actions rather than generic backhand, forehand, clear, drive, smash, or net-play evidence; when the user names either action, require direct action support instead of relying on an incidental transcript mention or a broad same-side source.
