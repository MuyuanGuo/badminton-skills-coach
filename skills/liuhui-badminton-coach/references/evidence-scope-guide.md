...
In a clarification continuation, derive hard focus only from the original user-authored question units. Treat user replies as reported observations or scenario constraints, and treat assistant-authored `query_label` text as audit metadata only. A reply may strengthen or weaken a structured conditional mechanism without requiring one source to prove the user's complete personal causal chain. Do not turn an absent exact phrase match into “no related video” when current-turn evidence still supports a bounded mechanism claim.
