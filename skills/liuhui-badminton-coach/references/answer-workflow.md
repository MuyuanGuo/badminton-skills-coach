...
- `actor_context.target_actor` identifies whether the requested advice targets the user or partner; use the resolved actor context for `他/她` and for training feeders such as a practice partner or shuttle machine. `question_interpretation.constraints` is the hard evidence scope for the answer target and may combine explicit target conditions with `derived_target_constraints`, such as an implicit doubles scene for rotation, coverage, or positioning. Do not describe those inferred scene constraints as strokes performed by the target. `opponent_constraints`, `partner_constraints`, and other non-target actor constraints remain stated conditions, not hard matches against the target's technique. Use `derived_player_constraints` and `derived_search_terms` only for configured user responses such as receiving another actor's serve or defending another actor's shot.
