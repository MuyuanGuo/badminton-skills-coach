...
- `question_interpretation.constraints` describes the target player's action. `actor_context.opponent_constraints` describes the opponent or incoming shot and must remain a stated condition, not a hard match against the target player's technique. Use `derived_player_constraints` and `derived_search_terms` only for configured responses such as receiving an opponent's serve or defending an opponent's shot.
