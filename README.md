...
回答质量修复依次检查五层：问题意图、同义/对照关系、证据选择、最终答案快照和盲测前向回答。真实用户失败必须先进入单题黄金集、关系负例和关键答案快照，再调整规则；新错误类型先扩展评测方法，不能只追加一个短语别名。盲测需保存原始回答和逐项审核记录，并绑定整个 Skill 运行时指纹；运行时变化后旧结果自动失效。日常改动先跑受影响的确定性门禁，合并前再跑完整流水线和该语义族的盲测。
...
- 分类误判：修改分类配置并补回归测试。
- 回答质量不足：先定位问题理解、来源解释、检索还是答案组织；同时补单题黄金集和同义问法关系回归，新错误类型先扩展评测方法再改规则。
- 用户反馈：本地信号只服务当前环境；公共信号必须经过脱敏、来源检查和版本发布。
- 课程或直播等新来源：沿用来源中立的 `evidence_id`、`source_type`、`parent_source_id` 与切片起止时间，并单独审核来源和授权边界；不要把整场直播当作一个不可定位的证据条目。
