...
| 指标 | 当前值 | 约束 |
| --- | ---: | --- |
| 已处理公开视频 | 1245 | 来源处理记录，不等同于全部教学内容 |
| B 站完整来源目录 | 767 | 599 条回答就绪、168 条策略排除或质量隔离、0 条待处理 |
| 可用于回答的教学视频 | 956 | 仅 `processing_status: ready` 进入证据池 |
...
性能门禁使用 5 类问题的平衡样本，限制模块加载、查询规划、搜索、回答上下文和峰值内存。扩展到 956 条可回答证据后的本地验收中，搜索 P95 为 `182.52 ms`、回答上下文 P95 为 `1,453.24 ms`、峰值追踪内存为 `176.50 MB`，回答包平均缩减 `82.05%`；均在 `750 ms`、`2,500 ms`、`192 MB` 和至少缩减 `50%` 的门禁内。这些是开发环境测量，不是跨机器性能承诺。
...
flowchart TD
    ...
    P --> E["结构化知识库（含隔离审计记录）"]
    E --> A["回答资格分层<br/>primary / supplemental / none"]
    A --> KG["概念-主题-证据角色图谱<br/>17,687条关系"]
    A --> F["45秒 chunk-first + 受限窗口检索<br/>跨来源内容簇与cluster-aware DF"]
...
新增转写不会写入模型权重或成为 Codex 的会话记忆。它只有先通过合集准入或独立来源核验，再通过媒体完整性、转写配方与 ASR 质量、来源文本安全、证据抽取和去重，才会获得回答资格。完整通过的记录成为 `primary`；仅标题与正文词面不一致、但直接教学证据合格的记录成为 `supplemental`，标题降为弱召回元数据，只能使用实际命中的时间戳窗口；硬质量、来源、安全或重复失败仍为 `none`。随后还必须通过图谱、索引、回归、canary、回答包预算和 Skill 安装，才可能影响当前问题。原始 `.json`、`.srt` 或 `.txt` 文件单独存在不会改变回答。
...
单条记录未通过来源、阻断级转写质量、来源安全、自动证据或重复门禁时，会保留审计状态并保持 `answer_eligibility: none`。标题与正文的词面不一致是降级信号而不是自动删除信号：只有在其他硬门禁全部通过时，记录才可作为受限补充证据进入，并且必须命中已提交窗口。跨产物不变量、稳定语料回归或发布门失败时才回滚本轮生成产物。
...
视频展示没有“整篇最多 3 条”的限制：单个结论最多映射 3 条最强证据，复杂问题会因独立子问题、实质场景分支或互补证据角色展示更多视频，整篇仍受 16 条答案候选硬上限、内容簇去重和逐结论证据门约束。紧凑包中的全部 `selected_videos` 都必须展示一次；回答可见视频会按用途重新连续编号为 `V1...Vn`。反馈记录器同时保留历史回答中 `V2、V3、V5` 等稀疏标签的原始映射，避免为追求连续编号而错绑视频。
