...
回答质量修复采用两层回归：先把真实问题加入单题黄金集，锁定直接证据、硬负例和答案边界；再把教学问法、症状问法和自然语言改写加入 `data/evaluation/query_equivalence_cases.json`，检查目标主体、规范动作、专项范围和共享核心视频，并为允许变化的症状保留单独断言。若新问题暴露的是此前没有评测的错误类型，应先扩展评测方法和负例，再调整规则，最后运行完整流水线；不能只追加一个短语别名。
