...
| B 站完整来源目录 | 767 | 589 条回答就绪、178 条策略排除或质量隔离、0 条待处理 |
| 可用于回答的教学视频 | 946 | 仅 `processing_status: ready` 进入证据池 |
| 主证据 / 受限补充证据 | 779 / 167 | 主证据优先；补充证据只使用命中的时间戳窗口 |
| 转写证据 | 760 | 7,697/7,697 条转写证据包含时间戳 |
| 受限时间戳窗口证据 | 167 | 1,792 条已提交窗口；标题不得作为结论证据 |
...
性能门禁使用 5 类问题的平衡样本，限制模块加载、查询规划、搜索、回答上下文和峰值内存。扩展到 946 条可回答证据后的本地验收中，搜索 P95 为 `232.11 ms`、回答上下文 P95 为 `1,552.47 ms`、峰值追踪内存为 `175.79 MB`，回答包平均缩减 `81.87%`；均在 `750 ms`、`2,500 ms`、`192 MB` 和至少缩减 `50%` 的门禁内。这些是开发环境测量，不是跨机器性能承诺。
...
flowchart TD
    ...
    A --> KG["概念-主题-证据角色图谱<br/>17,589条关系"]
