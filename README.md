...
**2.1.3 稳定版**来自 `main` 和 [v2.1.3 Release](https://github.com/MuyuanGuo/badminton-skills-coach/releases/tag/v2.1.3)。本项目独立开发，不是刘辉本人，也不代表刘辉或任何视频发布者的认可或背书。

**Version 2.1.3 is the stable release** from `main` and [v2.1.3](https://github.com/MuyuanGuo/badminton-skills-coach/releases/tag/v2.1.3). This independent project is not authored, operated, endorsed, or approved by Liu Hui or any source publisher.
...
stage="$(mktemp -d)"
unzip -q "/tmp/liuhui-badminton-coach-2.1.3.zip" -d "$stage"
...
~~~powershell
$base = "https://github.com/MuyuanGuo/badminton-skills-coach/releases/download/v2.1.3"
Invoke-WebRequest "$base/liuhui-badminton-coach-2.1.3.zip" -OutFile "$env:TEMP/liuhui-badminton-coach-2.1.3.zip"
...
$expected = ((Select-String "liuhui-badminton-coach-2.1.3.zip" "$env:TEMP/SHA256SUMS.txt").Line -split '\s+')[0]
$actual = (Get-FileHash "$env:TEMP/liuhui-badminton-coach-2.1.3.zip" -Algorithm SHA256).Hash.ToLower()
if ($actual -ne $expected) { throw "SHA-256 mismatch" }
$stage = Join-Path $env:TEMP "liuhui-skill-install"; Expand-Archive "$env:TEMP/liuhui-badminton-coach-2.1.3.zip" $stage -Force
...
- `main` 是稳定发布来源；当前稳定版为 `v2.1.3`。
- `main` is the stable release source; the current stable version is `v2.1.3`.
