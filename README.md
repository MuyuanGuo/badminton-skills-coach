...
面向 Codex 的证据驱动羽毛球教练 Skill。项目把刘辉羽毛球的公开视频处理成可检索、可引用、可回归验证的知识系统，再由问答编排器将自然语言问题约束到正确的技术、场区、角色和动作证据。

你正在查看 `develop` 分支。当前开发版本是 **1.2.0-dev.1**，发布状态为 **unreleased**。稳定使用请查看 [`main`](https://github.com/MuyuanGuo/badminton-skills-coach/tree/main) 和正式 [Release](https://github.com/MuyuanGuo/badminton-skills-coach/releases)。本项目不是刘辉本人，也不代表其个人观点或背书。
...
```text
Douyin profile observation
        |
        v
incremental snapshot -> classification ledger -> processing queue
        |                                      |
        v                                      v
download + metadata validation          transcript / visual review
        \                                      /
         v                                    v
      knowledge base -> topic map + retrieval index + quality signals
                                      |
                                      v
question -> intent/boundary parser -> exhaustive recall -> finalist selector
                                      |
                                      v
                    evidence-ready answer context for Codex Skill
                                      |
                                      v
       user feedback -> review queue -> regression cases -> rule/index update
```
...
- `v1.2.0`：当前正式 Release；develop 上的 README、实验性规则和未发布评测不代表稳定版行为。
